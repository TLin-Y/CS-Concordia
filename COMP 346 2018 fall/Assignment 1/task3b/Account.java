 ...
 * Because the withdraw value is ''20'', therefore with random unpredicted order, the final balance for 10 accounts are
 * random positive value here.
 * Here set a condition ''IF'' to protects bank from being overcharged.
 */
		if (balance - amount >= 0)       //When balance - amount < 0, end the thread without changing 'balance'.

			balance = balance - amount;
/**
 *  Do the ''If'' for all WithDraw actions
 */
		// Waste some time doing fake computations
		// do not remove or modify any of the following 3 statements
		k = 999999999;
